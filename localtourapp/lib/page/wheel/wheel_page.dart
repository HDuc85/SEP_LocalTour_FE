...
class _WheelPageState extends State<WheelPage> {
  ...
  List<String> _listColorItems = ['3a86ff','8338ec','ff006e','fb5607','ffbe0b'];
  int index = 0;
  ...
  Future<void> fetchInit() async {
    ...
    final fetchedListPlace = await _placeService.getListPlace(lat, long, SortBy.distance,SortOrder.asc,[],'',1,20);
  ...
  void _generateCardInfoList() async {
    final fetchedListPlace = await _placeService.getListPlace(_currentPosition.latitude, _currentPosition.longitude, SortBy.distance,SortOrder.asc,selectedTags,'',1,20);
    setState(() {
      _listPlaceCard = fetchedListPlace;
    });
  }
    ...
    if(isFirst){
      ...
      showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            title: Text(_languageCode == 'vi'?'Thần linh chọn cho ta!': "God chooses for us!"),
            content: Text('${_listPlaceCard[index].placeName}', style: TextStyle(color: Colors.red,fontWeight: FontWeight.bold, fontSize: 35),),
            actions: [
              TextButton(
                onPressed: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => DetailPage(placeId: _listPlaceCard[index].placeId),
                    ),
                  );
                },
                child: Text(_languageCode == 'vi' ? 'Tới trang chi tiết' : 'Go to detail'),
              ),
              TextButton(
                onPressed: () {
                  setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_languageCode == 'vi' ? "Hôm nay ăn gì?" : "What to eat today?"),
      ),
      body:
          isLoading?
